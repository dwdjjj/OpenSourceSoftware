...
#flg = plt.figure() new window => show new one seperate result
...
if __name__ == '__main__':
    ...
    plt.figure()
    plt.hist(total_kr, range=(0, 100), bins=20, density=False, cumulative=False,
             color='r', label='Korean')
    plt.hist(total_en, range=(0, 100), bins=20, density=False, cumulative=False,
             color='b', alpha=0.3, label='English')
    plt.ylabel('The number of students')
    plt.xlabel('Total scores')
    plt.xlim(0, 100)
    #plt.ylim(0, 9)
    plt.legend()
    #plt.savefig('class_score_hist_test.png')
    
    #plt.figure()
    #plt.hist(total_kr, range=(0,100), bins=20, edgecolor='black', density=True, cumulative=True, color='r', label='Korean')
    #plt.hist(total_en, range=(0,100), bins=20, edgecolor='black', density=True, cumulative=True, color='b', histtype='step', label='English')
