...
class RunawayGame:
    ...
    def __init__(self, canvas, runner, chaser, catch_radius=30, init_dist=400):
        self.canvas = canvas
        self.runner = runner
        self.chaser = chaser
    ...
    def step(self):
        ...
        if (self.chaser.pos()[1] < -300 or self.chaser.pos()[1] > 300):
            self.chaser.hideturtle()
            self.chaser.setx(0)
            self.chaser.sety(0)
            self.chaser.showturtle()                
        ...
        if(is_catched):
            score+=1
        self.drawer.write(f'Is catched? {is_catched} / Time: {elapse:.0f} / Score:{score:}')
        self.canvas.ontimer(self.step, self.ai_timer_msec)
